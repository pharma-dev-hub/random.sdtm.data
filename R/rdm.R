...
rdm <- function(n_patients = 400,
  ...
  dm$RFICDTC <- as.character(study_start - lubridate::days(with_seed(sample(seed, 0:30, n_patients, replace = TRUE))))
  dm$RFSTDTC <- as.character(lubridate::ymd(dm$RFICDTC) + lubridate::days(with_seed(sample(seed, 1:7, n_patients, replace = TRUE))))
  ...
  status <- with_seed(sample(seed, names(status_probs), n_patients, replace = TRUE, prob = status_probs))
  ...
  death_idx <- with_seed(sample(seed, seq_len(n_patients), size = ceiling(n_patients * 0.005)))
  ...
  dm$SITEID <- with_seed(sample(seed, sites, n_patients, replace = TRUE))
  ...
  # Sex, race, ethnicity
  dm$SEX <- with_seed(sample(seed, names(sex_probs), n_patients, replace = TRUE, prob = sex_probs))
  dm$RACE <- with_seed(sample(seed, names(race_probs), n_patients, replace = TRUE, prob = race_probs))
  dm$ETHNIC <- with_seed(sample(seed, names(ethnic_probs), n_patients, replace = TRUE, prob = ethnic_probs))
  ...
  dm$ACTARM <- dm$ARM
  crossover_idx <- with_seed(sample(seed, seq_len(n_patients), size = ceiling(n_patients * 0.02)))
  for (idx in crossover_idx) {
    new_arm <- with_seed(sample(seed, arm_codes[arm_codes != dm$ARMCD[idx]], 1))
    dm$ACTARMCD[idx] <- new_arm
    dm$ACTARM[idx] <- arms[[which(arm_codes == new_arm)]]$description
  }

  # Country
  dm$COUNTRY <- with_seed(sample(seed,
