# R/rti.R
#' Create Random Trial Inclusion/Exclusion Criteria (TI) Dataset
#'
#' @description
...
rti <- function(domain = "TI",
  ...
  if (!is.null(tirl) & length(tirl) > 0) {
    ...
    tirl_df <- as.data.frame(tirl) %>%
               mutate(IETESTCD = sapply(strsplit(as.character(tirl), "\\|"), "[", 1),
               TIRL = sapply(strsplit(as.character(tirl), "\\|"), "[", 2))
