...
rta <- function(domain = "TA",
  ...
  if (!is.null(tabranch) & length(tabranch) > 0) {
    ...
    tabranch_df <- as.data.frame(tabranch) %>% 
                   mutate(ARMCD = sapply(strsplit(tabranch, "\\|"), "[", 1),
                          TAETORD = as.numeric(sapply(strsplit(tabranch, "\\|"), "[", 2)),
                          TABRANCH = sapply(strsplit(tabranch, "\\|"), "[", 3))
  ...
  if (!is.null(tatrans) & length(tatrans) > 0) {
    ...
    tatrans_df <- as.data.frame(tatrans) %>% 
                  mutate(ARMCD = sapply(strsplit(tatrans, "\\|"), "[", 1),
                         TAETORD = as.numeric(sapply(strsplit(tatrans, "\\|"), "[", 2)),
                         TATRANS = sapply(strsplit(tatrans, "\\|"), "[", 3))
...
# Sample calling of rta function
rta(armcd = c("ADA", "ADA", "ADA", "DER", "DER", "DER", "PLA", "PLA", "PLA"),
    ...
    taetord = c(1, 2, 2003, 1, 2, 2003, 1, 2, 2003),
    ...
    tatrans = c("DER|2|Dermavalimab Arm", "PLA|2003|Placebo Arm"),
