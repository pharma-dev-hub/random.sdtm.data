...
# rvs Function to create synthetic VS dataset
rvs <- function(domain = "VS",
                ...
                lat = c(),
                grpid = c(),
                sort_seq = c("STUDYID", "USUBJID", "VISITNUM", "VSTESTCD", "VSTEST", "VSDTC"),
                drop_vars = c()) {
  ...
  # No. of TESTCD/Units provided should be matched
  ...
  if (!is.null(tpt) & length(tpt) > 0) {
    ...
    tpt_df <- as.data.frame(tpt_in) %>% 
              mutate(testcd = sapply(strsplit(tpt_in, "\\|"), "[", 1),
                     tpt = sapply(strsplit(tpt_in, "\\|"), "[", 2),
                     tptn = as.numeric(sapply(strsplit(tpt_in, "\\|"), "[", 3)))
  ...
  if (length(grpid) > 0) {
    ...
    grpid_df <- as.data.frame(grpid) %>% 
                mutate(testcd = sapply(strsplit(grpid, "\\|"), "[", 1),
  ...
  unit_chk <- function(unit_var) {
    ...
    unit_chk_df <- test_df2 %>% 
                   rowwise() %>% 
                   mutate(allowed = list(unit_vs[[testcd]]),
    ...
    stop("Error: Invalid units found! Check below messages for details.")
    } 
  }
  ...
  df3 <<- df2 %>% 
         mutate(STUDYID = studyid,
                ...
                VSORRESU = ifelse(VSORRES != "" & !is.na(VSORRES), unit_org, NA_character_),
                ...
                VSSTRESU = ifelse(VSSTRESC != "" & !is.na(VSSTRESC), unit_std, NA_character_),
...
# Sample calling of rVS function
